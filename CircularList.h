...
DoubleNode<T>* CircularList<T>::find(int index) 
{
   ...
   if (index >= loc_pos)
   {
               dis_next = index - loc_pos;                      //distance without the bridge (next refs, positive)
               dis_prev = loc_pos + (sze - index);       //distance using the bridge (prev refs, negative)
   }
   else
   {
              dis_prev = index - loc_pos;                 //distance without the bridge (prev refs, negative)
              dis_next = sze - loc_pos + index;                //distance using the bridge (next refs, positive)
   }
